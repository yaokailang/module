...
class PurchaseSpider(CrawlSpider):
    ...
    def parse_detail(self, response):
        title = response.xpath('/html/body/div[2]/div[3]/div/div[3]/h1/a/span/text()').get()   #标题
        content = response.xpath('/html/body/div[2]/div[3]/div/div[3]//text()').getall()      #内容
        turnover = response.xpath('//*[@id="share"]/span[1]/text()').get()      #发布时间
        urll = response.url      #url
        item = GzPurchaseItem(title=title,content=content,turnover=turnover,urll=urll)
        yield item
